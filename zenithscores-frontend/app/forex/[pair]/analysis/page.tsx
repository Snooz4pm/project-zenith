...
export default async function ForexAnalysisPage({ params }: PageProps) {
    const { pair } = await params;

    // Fetch data
    const data = await getAnalysis('forex', pair.toUpperCase());
...
export async function generateMetadata({ params }: PageProps) {
    const { pair } = await params;
    return {
        title: `${pair.toUpperCase()} Forex Analysis | ZenithScore`,
        description: `Deep quantitative analysis for ${pair.toUpperCase()} currency pair.`,
    };
}

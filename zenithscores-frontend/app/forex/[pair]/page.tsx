                            ...
                            <div className="flex justify-between items-center mb-6">
                                <div>
                                    ...
                                    <div className={`text-sm font-bold flex items-center gap-1 ${isPositive ? 'text-green-400' : 'text-red-400'}`}>
                                        ...
                                        {Math.abs(forex.change_24h).toFixed(2)}%
                                        <span className="text-gray-500 font-normal ml-1">Today</span>
                                    </div>
                                </div>
                                ...
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        className={`px-3 py-1.5 text-xs font-bold rounded border transition-colors ${showScoreOverlay ? 'bg-blue-500/20 border-blue-500/50 text-blue-400' : 'border-gray-700 text-gray-500'}`}
                                    >
                                        ZENITH
                                    </button>
                            ...
                            <div className="flex-1 w-full min-h-0">
                                <ResponsiveContainer width="100%" height="100%">
                                    <ComposedChart data={history}>
                                        ...
                                        <XAxis dataKey="date" tick={{ fontSize: 10, fill: '#6B7280' }} tickLine={false} axisLine={false} minTickGap={30} />
                                        <YAxis yAxisId="rate" domain={['auto', 'auto']} orientation="right" tick={{ fontSize: 10, fill: '#6B7280' }} tickLine={false} axisLine={false} tickFormatter={(val) => val.toFixed(4)} />
                                        {showScoreOverlay && <YAxis yAxisId="score" domain={[0, 100]} orientation="left" hide />}
                                        <Tooltip contentStyle={{ backgroundColor: '#1A1A22', border: '1px solid #374151', borderRadius: '8px' }} labelStyle={{ color: '#9CA3AF' }} />
                                        {showScoreOverlay && <Area type="monotone" dataKey="score" yAxisId="score" stroke="none" fill="#8B5CF6" fillOpacity={0.15} />}
                                        <Area type="monotone" dataKey="rate" yAxisId="rate" stroke="#3B82F6" strokeWidth={2} fill="url(#gradientRate)" />
                                    </ComposedChart>
                                </ResponsiveContainer>
                            </div>
                        </div>

...
import { GateDecision } from '@/lib/gate/engine';
...
export function DisciplinePanel({ isOpen, onClose }: DisciplinePanelProps) {
    const {
        isLocked,
        gateLevel,
        serverState,
                        ...
                        <div className="p-6 space-y-6">
                                ...
                                <div className="relative">
                                    <div className="h-4 bg-white/[0.05] rounded-full overflow-hidden">
                                        <motion.div
                                            ...
                                            className={`h-full rounded-full ${readinessIndex >= 70
                                    ...
                                    <div className="flex justify-between mt-2">
                                        ...
                                        <span className={`text-lg font-bold ${readinessIndex >= 70 ? 'text-emerald-400' :
                                            readinessIndex >= 40 ? 'text-yellow-400' : 'text-red-400'
                                            }`}>
                                            {readinessIndex}%
                                        </span>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="space-y-2">
                                        {localDecision.recommendations.map((rec, idx) => (
                                            <div
                                                key={idx}
                                                className="flex items-start gap-2 text-sm text-gray-300 bg-white/[0.02] p-3 rounded-lg border border-white/[0.06]"
                                            >
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex items-center gap-3 p-3 bg-white/[0.02] rounded-lg border border-white/[0.06]">
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${gateLevel === 'expert' ? 'bg-purple-500/20 text-purple-400' :
                                        gateLevel === 'pro' ? 'bg-blue-500/20 text-blue-400' :
                            ...
                            <div className="flex gap-3 pt-4 border-t border-white/[0.06]">
                                <button
                                    onClick={refresh}
                                    className="flex-1 py-2.5 px-4 bg-white/[0.03] hover:bg-white/[0.06] border border-white/[0.06] rounded-lg text-sm font-medium text-gray-300 transition-colors"
                                >
                                    Refresh
                                </button>
                                <button
                                    onClick={onClose}
                                    className="flex-1 py-2.5 px-4 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/20 rounded-lg text-sm font-medium text-emerald-400 transition-colors"
                                >
                                    Minimize
                                </button>
                            </div>

                            {/* Override (Pro/Expert only) */}
                            {status === 'locked' && (gateLevel === 'pro' || gateLevel === 'expert') && (
                                <div className="pt-2">
                                    <button
                                        onClick={() => override('User override requested')}
                                        className="w-full py-2.5 px-4 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 rounded-lg text-sm font-medium text-red-400 transition-colors"
                                    >
                                        Override Lock (5s wait)
                                    </button>
                                    <p className="text-[10px] text-gray-600 text-center mt-2">
                                        Overrides are logged and count against your discipline score
                                    </p>
                                </div>
                            )}
                        </div>

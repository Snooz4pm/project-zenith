...
export default function StudyWorkspace({ symbol, assetType, currentPrice, marketSignals = [] }: StudyWorkspaceProps) {
    ...
    // Theme State
    type Theme = 'terminal' | 'study' | 'paper' | 'focus';
    const [activeTheme, setActiveTheme] = useState<Theme>('terminal');

    // Theme Configurations
    const THEMES = {
        terminal: { bg: 'bg-[#0a0a0c]', text: 'text-zinc-200', border: 'border-white/5', header: 'text-white' },
        study: { bg: 'bg-[#18181b]', text: 'text-[#e4e4e7]', border: 'border-zinc-800', header: 'text-zinc-100' },
        paper: { bg: 'bg-[#fcfcfc]', text: 'text-[#1a1a1a]', border: 'border-zinc-200', header: 'text-zinc-900' },
        focus: { bg: 'bg-[#121212]', text: 'text-zinc-300', border: 'border-transparent', header: 'text-transparent hover:text-zinc-500 transition-colors' },
    };
    ...
    async function handleSaveNote() {
        ...
        // Apply prefix if set via UI or simple detection
        if (activeBlockType === 'h1') finalContent = `[H1] ${finalContent}`;
        else if (activeBlockType === 'hypothesis') finalContent = `[HYPO] ${finalContent}`;

        // Also support Slash Commands inline
        if (finalContent.startsWith('/h1 ')) finalContent = finalContent.replace('/h1 ', '[H1] ');
        if (finalContent.startsWith('/hypo ')) finalContent = finalContent.replace('/hypo ', '[HYPO] ');
        if (finalContent.startsWith('/rule ')) finalContent = finalContent.replace('/rule ', '[RULE] ');

        setIsSaving(true);
        // Append as an update for now (in future: Block based persistence)
        await appendMissionUpdate(currentMission.id, session.user.id, finalContent, currentPrice, 'study_workspace');
        setEditorContent('');
        setActiveBlockType('text'); // Reset
    ...
    return (
        <AnimatePresence>
            <motion.div
                ...
                exit={{ opacity: 0, y: 20 }}
                className={`fixed inset-0 z-50 backdrop-blur-xl flex justify-center overflow-hidden ${activeTheme === 'paper' ? 'bg-white/95' : 'bg-[#0a0a0c]/95'}`}
            >
                {/* Main Canvas Container (Centered, Notion-width) */}
                <div className={`w-full max-w-4xl h-full flex flex-col relative shadow-2xl transition-colors duration-500 ${currentTheme.bg} ${currentTheme.border} border-x`}>
                    ...
                    <header className="h-14 flex items-center justify-between px-6 border-b border-white/5">
                        ...
                        <div className="flex bg-white/5 rounded-lg p-0.5 border border-white/5">
                            {(['terminal', 'study', 'paper', 'focus'] as Theme[]).map(t => (
                                <button
                                    key={t}
                                    onClick={() => setActiveTheme(t)}
                                    className={`w-6 h-6 flex items-center justify-center rounded transition-all ${activeTheme === t ? 'bg-white/10 text-white shadow-sm' : 'text-zinc-600 hover:text-zinc-400'}`}
                                    title={t.charAt(0).toUpperCase() + t.slice(1)}
                                >
                                    <div className={`w-2 h-2 rounded-full ${t === 'paper' ? 'bg-white' : t === 'terminal' ? 'bg-emerald-500' : t === 'focus' ? 'bg-red-500' : 'bg-zinc-400'}`} />
                                </button>
                            ))}
                        </div>
                            ...
                            <div className="space-y-8 animate-in fade-in duration-500">
                                ...
                                <div className="space-y-6">
                                    {currentMission.missionUpdates.map((update, idx) => {
                                        // Simple Block Parser
                                        let content = update.note;
                                        let type = 'text';

                                        if (content.startsWith('[H1] ')) { type = 'h1'; content = content.replace('[H1] ', ''); }
                                        else if (content.startsWith('[H2] ')) { type = 'h2'; content = content.replace('[H2] ', ''); }
                                        else if (content.startsWith('[HYPO] ')) { type = 'hypo'; content = content.replace('[HYPO] ', ''); }
                                        else if (content.startsWith('[RULE] ')) { type = 'rule'; content = content.replace('[RULE] ', ''); }

                                        return (
                                            <div key={idx} className={`group relative pl-4 border-l-2 hover:border-zinc-500 transition-colors py-1 ${activeTheme === 'paper' ? 'border-zinc-300' : 'border-zinc-800'} ${type === 'hypo' ? 'border-l-blue-500 bg-blue-500/5 -ml-1 pl-5 rounded-r' : ''}`}>
                                                <div className={`leading-relaxed whitespace-pre-wrap font-serif ${currentTheme.text} ${type === 'h1' ? 'text-2xl font-bold' : type === 'h2' ? 'text-xl font-bold opacity-90' : 'text-base'}`}>
                                                    {content}
                                                </div>
                                                {type === 'hypo' && <div className="text-[10px] font-bold text-blue-400 mt-1 uppercase tracking-wider">HYPOTHESIS</div>}
